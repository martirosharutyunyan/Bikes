...
router.post('/add', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (let key in req.files) {
            // @ts-ignore
            arr = [...arr, Tools_1.generateFile(req.files[key].name, req.files[key].data)];
        }
...
router.post('/edit', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        arr = arr.map(elem => {
            if (files) {
                if (!elem.img.length) {
                    return {
                        id: +elem.id,
                        imagePath: [Tools_1.generateFile(elem.img.name, elem.img.data)]
                    };
                }
                const urls = elem.img.map(image => Tools_1.generateFile(image.name, image.data));
                return {
                    id: +elem.id,
                    imagePath: urls
                };
        ...
        data.forEach(elem => {
            ...
            if (image) {
                JSON.parse(elem.imagePath).forEach(elem => Tools_1.deleteImage(elem));
